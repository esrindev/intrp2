...
//std::vector<band_list_el> exec_band(BAND_SIZE);
...
int exec_decrement_opr(struct band_list_el *bll){
...
int exec_increment_opr(struct band_list_el *bll){
...
int exec_put_func_ret_res(struct band_list_el *bll){
...
/* get val from funk_ret_res and push to valstk   */
int exec_get_func_ret_res(struct band_list_el *bll){
...
int exec_crg2_adrs(struct band_list_el *bll){
  ...
  adrs += ((ind1 - 1)* max_col + ind2 - 1)*size;
  push_to_stek(&adrstk, adrs);
...
int exec_capet_float(struct band_list_el *bll){
  ...
  if ( p != NULL){
	q = p + 8; /* print only last 8 digits after dot  */
	if ( q > str + 50) /* check overflow  */
		q = str + 50 - 1;

	while ( q >= p && ( *q == '0' || *q == '\0' )) /* remove last zeroes  */{
		*q = '\0';
		q--;
	}

	if ( p == q ) /* remove dot if all digits after it was zeroes  */
      *q = '\0';
  }
...
/* POP from FNSTK which was pushed durion FCALL
*/
int exec_pop_fnstk( struct band_list_el *bll){
...
int exec_hal1(struct band_list_el *bll){
 ...
 if (tip == 1)/*int*/
	 sec_val = (int)get_value(adrs, 1, sizeof(long int));
		...
		sec_val = (char)get_value(adrs, 1, sizeof(long int));
 else
	 /* not supported type as switch variable*/
	 printf("Sehv: secim operatorunun deyiseni ancaq tam veya simvol tipli olmalidir.");
...
/* get value of counter from stack and comare with 0  */
int exec_check_say_tip_1(struct band_list_el *bll){
...
int exec_step_say_tip_1(struct band_list_el *bll){
...
int exec_init_say_tip_2(struct band_list_el *bll){
  ...
  /* !!! do not free ptr popped from adrstk   */ 
   ...
   //init loop operator counter to 1
  set_val(adrs, 1, tip);
...
int exec_check_say_tip_2(struct band_list_el *bll){

  unsigned char *adrs;
  long double val;
  void *ptr;
  int tip, size, sayqac, dovr_son;
...
int exec_step_say_tip_2(struct band_list_el *bll){
...
int exec_init_say_tip_3(struct band_list_el *bll){
...
int exec_check_say_tip_3(struct band_list_el *bll){

  unsigned char *adrs;
  long double val;
  void *ptr;
  int tip, size, sayqac, dovr_ilk, dovr_son;
...
int exec_step_say_tip_3(struct band_list_el *bll){
...
/* implementation of standart funcs, later possibly they will
 be moved to another file*/
int exec_kvadrat_kok(struct band_list_el *bll){

  /* get val from valstk  */
  long double  *ptr, val, sqr_root;
...
int pre_exec_new(struct band_list_el *head){
  ...
  case CHECK_SAY_TIP_1:
    return exec_check_say_tip_1(bll);
  case STEP_SAY_TIP_1:
    return exec_step_say_tip_1(bll);
  case NEQEDERKI2:
  ...
  case DECREMENT_OPR:
	  return exec_decrement_opr(bll);
  case INCREMENT_OPR:
	  return exec_increment_opr(bll);
  case NUMB:
  ...
  case PUT_FUNC_RET_RES:
    return exec_put_func_ret_res(bll);
 case GET_FUNC_RET_RES:
    return exec_get_func_ret_res(bll);
  case IDT_OFST:
  ...
  case SON:
    return exec_son(bll);
  case INIT_SAY_TIP_2:
	  return exec_init_say_tip_2(bll);
  case INIT_SAY_TIP_3:
	  return exec_init_say_tip_3(bll);
  case CHECK_SAY_TIP_2:
    return exec_check_say_tip_2(bll);
  case STEP_SAY_TIP_2:
    return exec_step_say_tip_2(bll);
  case CHECK_SAY_TIP_3:
    return exec_check_say_tip_3(bll);
  case STEP_SAY_TIP_3:
    return exec_step_say_tip_3(bll);
 




	/* STANDART FUCS GO HERE . . .  */
  case KVADRAT_KOK:
	  return exec_kvadrat_kok(bll);
...
int main_old (){
  ...
  /* sorry guys, I just ignored ugly parsing work, so things ended with 
     main funcs(program) local variables which are also programs global variables gatehered as 
	 members of global object, so we have to give referense to it from main
  */
  /*  main->dt.count = 0; */
 ...
 // print_exec_band(exec_band, band_size);
  ...
  /* init glb tips area  */
	...
	/*before start we push main's ret adrs i.e. exit adrs to 
	  glbstk. So all rets from main will cause process exit
	*/
	prg_exit = exec_band[0].head_pos;
	push_to_stek(&glbstk, (void *)(&prg_exit));
